...
class CLIPMoCoWrapper(nn.Module):
    def __init__(self, clip_model, embed_dim, queue_size=65536, momentum=0.999, temperature=0.07):
        super().__init__()
    ...
    def encode_text(self, text):
        """
        Encode text inputs using the query encoder's encode_text method.

        Parameters:
        - text: a batch of text inputs.

        Returns:
        - Text features encoded by the query encoder.
        """
        return self.query_encoder.encode_text(text)
...
def model_pipeline(config, verbose=0): 
    ...
    # save model
    model_path = os.path.join(config.save_dir, str(config.model_name), 'checkpoint_pretraining_moco_32_Closs_run6.pt')
...
def make(config):
    ...
    #Initialize MoCo wrapper
    moco_wrapper = CLIPMoCoWrapper(clip_model=model, embed_dim=512)  
